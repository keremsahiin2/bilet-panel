...
export default function App() {
  ...
  if (mode === 'quiz' || role === 'quiznight') {
    ...
    if (quizStep === 'qcount') {
      // Soru sayısını dosyadan otomatik tespit et
      const detectedQ = Object.keys(quizQuestions).length;
      const autoCount = detectedQ === 50 ? 50 : detectedQ === 55 ? 55 : null;

      // Dosya yüklü ve 50 veya 55 soruysa otomatik devam et
      if (autoCount !== null) {
        QUIZ_EVENTS.genelkultur.totalQ = autoCount;
        setQuizGroups([]); setQuizGroupCountSet(false); setQuizGroupCount('');
        setQuizScores({}); setQuizMyGroups([]); setQuizCurrentQ(1);
        setQuizStep('groups');
        return null;
      ...
      return (
        <div style={S.page}>
          ...
          <div style={{maxWidth:480,margin:'0 auto',padding:'40px 18px',textAlign:'center'}}>
            ...
            {detectedQ > 0 && detectedQ !== 50 && detectedQ !== 55 && (
              <div style={{background:'#0d1a2e',border:'1px solid #4fc9ff44',borderRadius:10,padding:'10px 16px',marginBottom:16,fontSize:12,color:'#4fc9ff'}}>
                📁 Yüklenen dosyada {detectedQ} soru var
              </div>
            )}
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              {[50, 55].map(count => (
