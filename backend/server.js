...
async function createOneSeance(payload) {
  ...
  var seansSlugBase = (realParentSlug || '').slice(0, 80).replace(/-+$/,'');
  var seansSlug = (seansSlugBase ? seansSlugBase + '-' : '') + seansSlugSuffix;
  if (seansSlug.length > 220) seansSlug = seansSlug.slice(0, 220).replace(/-+$/,'');
...
app.post('/api/ideasoft/create-seances-bulk', async function(req, res) {
  ...
  }
  {
    ...
    try {
      ...
      while (true) {
        ...
        console.log('Bulk: optioned-products sayfa=' + opPage + ' ->', pageSeances.length, 'seans (toplam:', opSeances.length + ')');
        if (pageSeances.length < 100) break; // son sayfa
        opPage++;
        await new Promise(r => setTimeout(r, 300));
      }
      console.log('Bulk: optioned-products toplam', opSeances.length, 'seans,', Date.now() - t0bulk, 'ms');
  ...
  if (seansItems.length > 0) {
    ...
    // Per-seans batch yanit parse — sıra bazlı (İdeasoft Content-ID'yi değiştiriyor)
    // Her --boundary bloğunu ayır, sırasıyla seansItems ile eşleştir
    var batchBlocks = [];
    var boundaryMarker = '--' + boundary;
    var rawBlocks = batchStr.split(boundaryMarker);
    // İlk eleman boş, son eleman '--' ile biter — ikisini atla
    for (var bi = 1; bi < rawBlocks.length; bi++) {
      var block = rawBlocks[bi];
      if (block.trim() === '--' || block.trim() === '') continue;
      // HTTP status kodunu bul
      var httpM = block.match(/HTTP\/1\.\d (\d{3})/);
      var blockStatus = httpM ? parseInt(httpM[1]) : 0;
      // JSON body bul
      var jsonStart = block.indexOf('{');
      var blockJson = null;
      if (jsonStart !== -1) {
        try { blockJson = JSON.parse(block.slice(jsonStart)); } catch(e) {}
      }
      batchBlocks.push({ status: blockStatus, json: blockJson, raw: block });
    }
    console.log('Bulk: batch parse — toplam blok:', batchBlocks.length, 'beklenen:', seansItems.length);

    seansItems.forEach(function(item, idx) {
      var block = batchBlocks[idx];
      var succeeded = false;
      var errDetail = 'Batch yanitinda bulunamadi';

      if (block) {
        var statusCode = block.status;
        if (statusCode >= 200 && statusCode < 300) {
          succeeded = true;
        } else if (statusCode === 400) {
          var rawSnippet = block.raw.slice(0, 400);
          if (rawSnippet.includes('Tekrarlanan') || rawSnippet.includes('tekrarlanan')) {
            results.push({ success: false, skipped: true, name: item.payload.name, error: 'Zaten mevcut (atlandi)' });
            if (jobId && bulkProgressMap[jobId]) {
              bulkProgressMap[jobId].done++;
              bulkProgressMap[jobId].results.push({ success: false, skipped: true, name: item.tarihSaatTitle, error: 'Zaten mevcut' });
            }
            console.log('Bulk per-seans: ATLANDI (Tekrarlanan):', item.tarihSaatTitle);
            return;
          }
          // Hata mesajını JSON'dan al
          if (block.json && block.json.errors) {
            errDetail = 'HTTP 400: ' + JSON.stringify(block.json.errors).slice(0, 120);
          } else {
            errDetail = 'HTTP 400';
          }
        } else if (statusCode > 0) {
          errDetail = 'HTTP ' + statusCode;
        }
      }
      ...
      if (succeeded) {
        ...
        console.log('Bulk per-seans: OK:', item.tarihSaatTitle);
      } else {
        results.push({ success: false, name: item.payload.name, error: errDetail });
        if (jobId && bulkProgressMap[jobId]) {
          bulkProgressMap[jobId].done++;
          bulkProgressMap[jobId].errors++;
          bulkProgressMap[jobId].results.push({ success: false, name: item.tarihSaatTitle, error: errDetail });
        }
        console.error('Bulk per-seans: HATA:', item.tarihSaatTitle, '|', errDetail);
      }
    });
    var successNow = results.filter(function(r){ return r.success; }).length;
    var skippedNow = results.filter(function(r){ return r.skipped; }).length;
    console.log('Bulk: batch tamamlandi — basarili:', successNow, '| atlanan:', skippedNow, '| hata:', results.filter(function(r){ return !r.success && !r.skipped; }).length);
  }
  ...
  var errorCount   = results.filter(function(r) { return !r.success && !r.skipped; }).length;

  // Hata varsa parentDataCache'i temizle — bir sonraki retry fresh optioned-products ceksin
  // 429 sonrasi bazi seanslar olusmus olabilir; eski cache'li Set bunlari bilmez
  if (errorCount > 0) {
    delete parentDataCache[parentId];
...
// ─── Mevcut seansları kontrol et (preview için) ───────────────────────────────
app.post('/api/ideasoft/check-existing-seances', async function(req, res) {
  if (!ideasoftCookies) return res.status(401).json({ error: 'İdeasoft oturumu yok' });
  var parentIds = req.body.parentIds;
  if (!Array.isArray(parentIds) || parentIds.length === 0) return res.json({ existingNames: [] });
  var cStr = toCookieStr(ideasoftCookies);
  var hdrs = {
    'Cookie': cStr, 'X-CSRF-TOKEN': ideasoftCsrfToken || '',
    ...
    for (var i = 0; i < parentIds.length; i++) {
      ...
      while (true) {
        var r = await axios.get(
          'https://berkayalabalik.myideasoft.com/admin-app/optioned-products/' + pid + '?page=' + page + '&limit=100&sort=-id&status=1',
          { headers: hdrs, timeout: 20000 }
        );
        var pageData = Array.isArray(r.data.data) ? r.data.data : Array.isArray(r.data) ? r.data : [];
        pageData.forEach(function(s) { if (s.name) existingNames.add(s.name.trim().toLowerCase()); });
        if (pageData.length < 100) break;
        page++;
        await new Promise(function(resolve) { setTimeout(resolve, 200); });
      }
      if (i < parentIds.length - 1) await new Promise(function(resolve) { setTimeout(resolve, 200); });
    }
    res.json({ existingNames: Array.from(existingNames) });
  } catch(e) {
    console.error('check-existing-seances hatasi:', e.message);
